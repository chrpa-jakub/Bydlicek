...
namespace LiveBot
{
    public class DumrealitParser : IParser
    {
        public Flat Parse(string url)
        {
            ...
            var priceRaw = doc.DocumentNode.SelectSingleNode("//span[contains(@class,'price')]").InnerHtml.Split(" ");
            var flatId = doc.DocumentNode.SelectSingleNode("//span[contains(@class,'id')]").InnerHtml.Split(" ")[1];
            var link = "https://www.dumrealit.cz"+doc.DocumentNode.SelectSingleNode("//a[contains(@class,'hovout')]").OuterHtml.Split("\"")[1];
