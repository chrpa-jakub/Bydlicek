        ...
        public Flat Parse(string url)
        {
            try
            {
                ...
                var link = doc.DocumentNode.SelectSingleNode("//a[@class='advert-list-items__images']").OuterHtml
                    .Split("\"")[1];
                var name = Regex.Replace(doc.DocumentNode.SelectSingleNode("//h2").InnerHtml.Split(">")[^2],
                    @"</a|\s{2,}", "");
                var locality = doc.DocumentNode.SelectSingleNode("//p[@class='advert-list-items__content-address']")
                    .InnerHtml;
                var price = int.Parse(Regex.Replace(doc.DocumentNode
                    .SelectSingleNode("//span[@class='advert-list-items__content-price-price']")
                    .InnerHtml, @"\s|Kč", ""));
