    ...
    public class MaxiRealityParser : IParser
    {
        public Flat Parse(string url)
        {
            try
            {
                ...
                var name = Regex.Replace(doc.DocumentNode.SelectSingleNode("//a[@class='text-red']").InnerHtml,
                    "&sup2;", "2");
                var size = name.Split(" ")[2];
                var locality = doc.DocumentNode.SelectSingleNode("//p[@class='fs-14 no-margin-bottom adress']")
                    .InnerHtml;
                var priceRaw = doc.DocumentNode.SelectSingleNode("//span[@class='big text-blue']").InnerHtml.Split(" ");
                var link = "https://www.maxirealitypraha.cz" +
                           doc.DocumentNode.SelectSingleNode("//a[@class='text-red']").OuterHtml.Split("\"")[1];
